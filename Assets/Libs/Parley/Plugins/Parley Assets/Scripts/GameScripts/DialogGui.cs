...
public class DialogGui : DialogGuiAbstract {
	...
	private bool DrawText(Dialog dialog){
		...
		GUILayout.Label(conversationText,new GUILayoutOption[]{GUILayout.ExpandHeight(true),GUILayout.ExpandWidth(true)});
	...
	private void DrawButtons(Dialog dialog){
    	...
    	foreach (Option o in options){
    		if (o._available){
				if (GUILayout.Button((options.IndexOf(o) + 1) + ") " + o.displaytext)) {
					GameManager.instance.RandomRoll();
		  			SelectOption(o);
					conversationTextStartSec=Time.time;
		  		}
		  	}// else {
			//	GUILayout.Button((options.IndexOf(o) + 1) + ") " + o.displaytext);
			//}
    	}
