...
public class Dialog : MonoBehaviour, QuestChangedListener {
	...
	public void TriggerDialog(int start){
		...
		Type[] scripts = this.GetType().Assembly.GetExportedTypes();

		if (scripts != null && scripts.Length > 0) {
			foreach (Type script in scripts) {
				if (script.IsSubclassOf(typeof(DialogGuiAbstract))) {
					if (script.Name == dialogClass) {
