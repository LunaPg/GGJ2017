...
public abstract class DialogGuiAbstract : MonoBehaviour {
	 ...
	 * 
	 */
	public void GotoDialogue(string dialogName,int index){
			...
			foreach (Option o in conversation.options){
				...
				if (odialog==null){
					...
					if (o.quest){
						...
						if (o.destinationId>=0 && o.destinationId<odialog.GetConversations().Length){
							...
							o._available=false;
							Debug.LogWarning("Parley: Conversation #"+conversation.uniqueId+" option "+optionNumer+" leads to Dialog #"+o.destinationId+" which can not be found in the scene.");
						}
					}
					if (o._available){
						availableOptions++;
	...
	protected List<Option> GetCurrentConversationOptions(){
		if (currentOptions==null && conversation.options!=null){
        	...
        	foreach (Option o in conversation.options){
				...
				//	o.displaytext=Parley.GetInstance().EmbedEnviromentalInformation(o.text);
				//	o.displaytext = o.displaytext + " (uncomplete)";
				//	currentOptions.Add(o);
				//}
        	}
		}
